...
import photoListStyle from "../../styles/PhotosList.module.css";

const List = ({ cats }) => {
  const router = useRouter();
  const { id } = router.query;
  return (
    <div className={photoListStyle.photos}>
      ...
      <ul>
        {cats.map((cat) => (
          <li key={cat.id}>
            ...
            />
            <div className={photoListStyle.info}>
...
// getStaticPaths 를 작성해야 함.
// /photos/id 를 전송시
// getStaticPahts 가 실행 된다.
// params 를 통해서 전달된 id를
// { params : { id : 문자열값} } 을
// 모은 배열(paths )에 담아서 context 로 전송.
// 이 context 는 getStaticProps 에서 활용
